...
class WineService {
  static List<Map<String, dynamic>> wines = [
    {
    ...
    },
     {
      ...
      "origin": "Argentina",
      "year": "2019",
      "flag": "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Flag_of_Argentina.svg/200px-Flag_of_Argentina.svg.png",
      "type": "Malbec",
      "grapes": "Malbec",
  ...
  static void removeWine(Map<String, dynamic> wine) {
    wines.remove(wine);
  }
...
class CartService {
  ...
  static void addItem(Map<String, dynamic> wine) {
    ...
    if (index >= 0) {
      ...
      Map<String, dynamic> newItem = Map.from(wine);
      newItem['qty'] = 1;
      items.add(newItem);
    }
  }
...
// ==========================================
// WIDGET REUTILIZÁVEL: INPUT DE VIDRO (CORRIGIDO)
// ==========================================
class _GlassInput extends StatelessWidget {
  ...
  // Removi o 'onChanged' que estava sobrando
  const _GlassInput({
    required this.label, 
...
class _SocialButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: onTap,
      child: Container(width: 55, height: 55, decoration: BoxDecoration(color: color, shape: BoxShape.circle), child: Center(child: FaIcon(icon, color: iconColor, size: 26))),
    );
  }
}
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _doLogin() {
    ...
    if (user == "Luffy" && pass == "2709") {
      SessionService.isAdmin = true;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Bem-vindo, Chefe Luffy! 🍷"), backgroundColor: Color(0xFF800020)));
    } else {
      SessionService.isAdmin = false;
    }
    Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const CatalogScreen()), (route) => false);
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _GlassInput(label: "Nome ou E-mail", controller: _userController), const SizedBox(height: 20),
                        _GlassInput(label: "Senha", isPassword: true, controller: _passController), const SizedBox(height: 30),
                        SizedBox(width: double.infinity, height: 55, child: OutlinedButton(onPressed: _doLogin, style: OutlinedButton.styleFrom(side: const BorderSide(color: Colors.white30), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: Text("ENTRAR", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, letterSpacing: 1.5)))),
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  void _handleSignup() {
    if (_nameController.text.isEmpty || _emailController.text.isEmpty || _dobController.text.isEmpty || _passController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Preencha todos os campos."), backgroundColor: Colors.redAccent)); return;
    }
    if (_passController.text != _confirmPassController.text) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Senhas não coincidem."), backgroundColor: Colors.redAccent)); return;
    }
    if (!_agreedToTerms) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Aceite os termos."), backgroundColor: Colors.redAccent)); return;
    }
    SessionService.isAdmin = false;
    Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const CatalogScreen()), (route) => false);
  }
...
// ==========================================
// TELA 4: CATALOG SCREEN (COM FILTROS AVANÇADOS)
// ==========================================
...
class _CatalogScreenState extends State<CatalogScreen> {
  // Lista de Vinhos Exibida (Pode ser filtrada)
  List<Map<String, dynamic>> displayWines = [];
  
  // Variáveis de Filtro
  String searchQuery = "";
  String? selectedCountry;
  String? selectedGrape;
  RangeValues priceRange = const RangeValues(0, 60000);

  @override
  void initState() {
    super.initState();
    displayWines = List.from(WineService.wines); // Começa com todos
  }

  // Lógica de Filtro
  void _applyFilters() {
    setState(() {
      displayWines = WineService.wines.where((wine) {
        // 1. Filtro por Nome
        final nameMatch = wine['name'].toString().toLowerCase().contains(searchQuery.toLowerCase());
        
        // 2. Filtro por País
        final countryMatch = selectedCountry == null || wine['origin'].toString() == selectedCountry;
        
        // 3. Filtro por Uva
        final grapeMatch = selectedGrape == null || wine['grapes'].toString().contains(selectedGrape!);

        // 4. Filtro por Preço (Parse R$)
        double price = double.parse(wine['price'].toString().replaceAll(RegExp(r'[^0-9]'), ''));
        final priceMatch = price >= priceRange.start && price <= priceRange.end;

        return nameMatch && countryMatch && grapeMatch && priceMatch;
      }).toList();
    });
    Navigator.pop(context); // Fecha o Drawer após filtrar
  }
  ...
  void _clearFilters() {
    setState(() {
      ...
      selectedGrape = null;
      priceRange = const RangeValues(0, 60000);
      displayWines = List.from(WineService.wines);
    });
    Navigator.pop(context);
  }

  void _deleteWine(Map<String, dynamic> wine) {
    showDialog(context: context, builder: (ctx) => AlertDialog(backgroundColor: const Color(0xFF1E1E1E), title: Text("Excluir Vinho?", style: GoogleFonts.poppins(color: Colors.white)), content: Text("Tem certeza que deseja excluir ${wine['name']}?", style: GoogleFonts.poppins(color: Colors.white70)), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("Cancelar")), TextButton(onPressed: () { setState(() { WineService.removeWine(wine); displayWines = List.from(WineService.wines); }); Navigator.pop(ctx); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vinho removido!"))); }, child: const Text("Excluir", style: TextStyle(color: Colors.redAccent)))]));
  }

  @override
  Widget build(BuildContext context) {
    // Pega listas únicas para os Dropdowns
    final countries = WineService.wines.map((e) => e['origin'].toString()).toSet().toList();
    final grapes = WineService.wines.map((e) => e['grapes'].toString()).toSet().toList(); // Simplificado: considera a string inteira da uva

    return Scaffold(
      backgroundColor: const Color(0xFF121212),
      // DRAWER DE FILTROS (O MENU LATERAL)
      drawer: Drawer(
            ...
            DrawerHeader(
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    const Icon(Icons.filter_list_alt, size: 40, color: Colors.white),
                    const SizedBox(height: 10),
                    Text("Filtros & Busca", style: GoogleFonts.greatVibes(fontSize: 30, color: Colors.white)),
                  ],
                ),
              ),
            ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: TextField(
                      ...
                      style: const TextStyle(color: Colors.white),
                      decoration: const InputDecoration(border: InputBorder.none, prefixIcon: Icon(Icons.search, color: Colors.white54), contentPadding: EdgeInsets.symmetric(vertical: 10)),
                    ),
                  ),
                  const SizedBox(height: 20),

                  // Filtro País
                  Text("País de Origem", style: GoogleFonts.poppins(color: Colors.white70, fontSize: 12)),
                  DropdownButton<String>(
                    value: selectedCountry,
                    isExpanded: true,
                    dropdownColor: const Color(0xFF2C2C2C),
                    hint: const Text("Todos os países", style: TextStyle(color: Colors.white54)),
                  ...
                  // Filtro Uva
                  Text("Tipo de Uva", style: GoogleFonts.poppins(color: Colors.white70, fontSize: 12)),
                  DropdownButton<String>(
                    value: selectedGrape,
                    isExpanded: true,
                    dropdownColor: const Color(0xFF2C2C2C),
                    hint: const Text("Todas as uvas", style: TextStyle(color: Colors.white54)),
                    items: grapes.map((String value) {
                      return DropdownMenuItem<String>(value: value, child: Text(value, style: const TextStyle(color: Colors.white)));
                    }).toList(),
                    onChanged: (val) => setState(() => selectedGrape = val),
                  ),
                  const SizedBox(height: 20),

                  // Slider de Preço
                  Text("Faixa de Preço: R\$ ${priceRange.start.toInt()} - R\$ ${priceRange.end.toInt()}", style: GoogleFonts.poppins(color: Colors.white70, fontSize: 12)),
                  RangeSlider(
                    values: priceRange,
                    min: 0,
                    max: 60000,
                    ...
                    labels: RangeLabels("R\$ ${priceRange.start.toInt()}", "R\$ ${priceRange.end.toInt()}"),
      ...
      appBar: AppBar(
        ...
        // O Botão Menu abre o Drawer automaticamente
        ...
        actions: [
          IconButton(icon: const Icon(Icons.exit_to_app, color: Colors.white), tooltip: 'Sair da conta', onPressed: () { CartService.clearCart(); Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const WelcomeScreen()), (route) => false); }),
          Padding(padding: const EdgeInsets.only(right: 10.0), child: IconButton(icon: const Icon(Icons.shopping_cart_outlined, color: Colors.white, size: 28), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const CartScreen()))))
        ],
      ),
      body: Stack(
        children: [
          ...
          Center(
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Destaques da Adega", style: GoogleFonts.poppins(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w600)), if (SessionService.isAdmin) Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.circular(4)), child: const Text("MODO ADMIN", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 10)))])),
                  
                  // LISTA DE VINHOS (USANDO A LISTA FILTRADA 'displayWines')
                  Expanded(
                    child: displayWines.isEmpty 
                    ? Center(child: Text("Nenhum vinho encontrado.", style: GoogleFonts.poppins(color: Colors.white54)))
                    : GridView.builder(
                      padding: const EdgeInsets.fromLTRB(15, 0, 15, 100), itemCount: displayWines.length,
                      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, childAspectRatio: 0.60, crossAxisSpacing: 15, mainAxisSpacing: 15),
                      itemBuilder: (context, index) {
                        final wine = displayWines[index];
                        return Stack(children: [GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => ProductDetailScreen(wine: wine))), child: _WineCard(wine: wine)), if (SessionService.isAdmin) Positioned(top: 5, right: 5, child: GestureDetector(onTap: () => _deleteWine(wine), child: Container(padding: const EdgeInsets.all(5), decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle), child: const Icon(Icons.delete, size: 16, color: Colors.white))))]);
                      },
                    ),
                  ),
                ],
      ...
      floatingActionButton: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          if (SessionService.isAdmin) Padding(padding: const EdgeInsets.only(bottom: 15.0), child: FloatingActionButton.extended(onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (context) => const AdminAddWineScreen())).then((_) => setState((){ displayWines = List.from(WineService.wines); })); }, backgroundColor: Colors.amber, icon: const Icon(Icons.add, color: Colors.black), label: const Text("Adicionar Vinho", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)))),
          Container(width: 200, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12), margin: const EdgeInsets.only(bottom: 10), decoration: BoxDecoration(color: const Color(0xFF1E1E1E), borderRadius: BorderRadius.circular(30), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.5), blurRadius: 10, spreadRadius: 2)], border: Border.all(color: Colors.white10)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [const Icon(Icons.home, color: Colors.white, size: 28), Container(width: 1, height: 25, color: Colors.white24), GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const CartScreen())), child: const Icon(Icons.shopping_cart_outlined, color: Colors.white, size: 28))])),
        ],
      ),
    );
  }
...
class _WineCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(color: const Color(0xFF1E1E1E), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withOpacity(0.05)), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.5), blurRadius: 8, offset: const Offset(0, 4))]),
      child: Column(children: [Expanded(flex: 3, child: Container(width: double.infinity, padding: const EdgeInsets.all(10), decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.vertical(top: Radius.circular(20))), child: Hero(tag: wine['name'], child: Image.network(wine['image'], fit: BoxFit.contain, errorBuilder: (c,e,s) => const Icon(Icons.wine_bar, size: 50, color: Colors.grey))))), Expanded(flex: 2, child: Padding(padding: const EdgeInsets.all(12.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(wine['name'], style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15), maxLines: 1, overflow: TextOverflow.ellipsis), Text("${wine['origin']} • ${wine['year']}", style: GoogleFonts.poppins(color: Colors.grey, fontSize: 11))]), Text(wine['price'], style: GoogleFonts.poppins(color: Colors.amber[700], fontWeight: FontWeight.bold, fontSize: 16))]))) ]),
    );
  }
}
...
class _AdminAddWineScreenState extends State<AdminAddWineScreen> {
  ...
  void _saveWine() {
    ...
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vinho adicionado ao catálogo!"), backgroundColor: Colors.green));
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 20), Text("Novo Produto", style: GoogleFonts.poppins(fontSize: 20, color: Colors.white, fontWeight: FontWeight.bold)),
            const SizedBox(height: 30),
            ...
            const SizedBox(height: 30),
            SizedBox(width: double.infinity, height: 55, child: ElevatedButton(onPressed: _saveWine, style: ElevatedButton.styleFrom(backgroundColor: Colors.amber), child: Text("SALVAR NO CATÁLOGO", style: GoogleFonts.poppins(color: Colors.black, fontWeight: FontWeight.bold))))
          ],
        ),
      ),
    );
...
// ==========================================
// TELA 6 e 7 (Carrinho e Pagamento)
// ==========================================
      ...
      body: Stack(
        children: [
          ...
          Column(children: [
            ...
            Container(margin: const EdgeInsets.symmetric(horizontal: 20), padding: const EdgeInsets.symmetric(vertical: 15, horizontal: 20), decoration: BoxDecoration(color: const Color(0xFF581C1C), borderRadius: BorderRadius.circular(20), boxShadow: const [BoxShadow(color: Colors.black45, blurRadius: 10)]), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [const Icon(Icons.shopping_cart_outlined, color: Colors.white, size: 28), const SizedBox(width: 10), Text("Carrinho", style: GoogleFonts.greatVibes(fontSize: 35, color: Colors.white))]), Text("${CartService.items.length} Itens", style: GoogleFonts.poppins(color: Colors.white70))])),
            const SizedBox(height: 20),
            Expanded(child: CartService.items.isEmpty ? Center(child: Text("Seu carrinho está vazio...", style: GoogleFonts.poppins(color: Colors.white54))) : ListView.builder(padding: const EdgeInsets.symmetric(horizontal: 20), itemCount: CartService.items.length, itemBuilder: (context, index) { return _CartItem(item: CartService.items[index], onChanged: () => setState((){})); })),
            Container(padding: const EdgeInsets.all(25), decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: const BorderRadius.vertical(top: Radius.circular(30)), border: Border(top: BorderSide(color: Colors.white.withOpacity(0.1)))), child: Column(children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Valor Total:", style: GoogleFonts.poppins(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), Text("R\$ ${totalValue.toStringAsFixed(2)}", style: GoogleFonts.poppins(color: Colors.greenAccent, fontSize: 20, fontWeight: FontWeight.bold))]), const SizedBox(height: 20), SizedBox(width: double.infinity, height: 55, child: ElevatedButton(onPressed: () { if (CartService.items.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Carrinho vazio!"))); } else { Navigator.push(context, MaterialPageRoute(builder: (context) => const PaymentScreen())); } }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF800020), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), child: Text("PAGAMENTO", style: GoogleFonts.poppins(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))))]))
          ]),
        ],
      ),
    );
...
class _CartItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(margin: const EdgeInsets.only(bottom: 15), padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white.withOpacity(0.15), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.white.withOpacity(0.1))), child: Row(children: [Container(width: 70, height: 90, padding: const EdgeInsets.all(5), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15)), child: Image.network(item['image'], fit: BoxFit.contain, errorBuilder: (c,e,s) => const Icon(Icons.wine_bar))), const SizedBox(width: 15), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item['name'], style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)), const SizedBox(height: 5), Text(item['price'], style: GoogleFonts.poppins(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 14))])), Column(children: [Row(children: [GestureDetector(onTap: () { CartService.removeItem(item); onChanged(); }, child: const Icon(Icons.remove_circle_outline, color: Colors.white70)), Padding(padding: const EdgeInsets.symmetric(horizontal: 10.0), child: Text("${item['qty']}", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold))), GestureDetector(onTap: () { CartService.addItem(item); onChanged(); }, child: const Icon(Icons.add_circle_outline, color: Colors.white))])])]));
  }
}
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      extendBodyBehindAppBar: true, appBar: AppBar(backgroundColor: Colors.transparent, elevation: 0, leading: const BackButton(color: Colors.white), centerTitle: true, title: Text("Formas de Pagamentos", style: GoogleFonts.greatVibes(fontSize: 35, color: Colors.white)), actions: [IconButton(onPressed: (){}, icon: const Icon(Icons.menu, color: Colors.white))]),
      body: Stack(
        children: [
          ...
          Column(children: [const SizedBox(height: 120), Expanded(child: ListView(padding: const EdgeInsets.symmetric(horizontal: 20), children: [_PaymentOptionRow(icon: FontAwesomeIcons.solidCreditCard, label: "Cartão de Crédito/Débito", isSelected: _selectedMethod == 1, onTap: () => setState(() => _selectedMethod = 1)), const SizedBox(height: 15), _PaymentOptionRow(icon: FontAwesomeIcons.pix, label: "Pix", isSelected: _selectedMethod == 2, onTap: () => setState(() => _selectedMethod = 2)), const SizedBox(height: 15), _PaymentOptionRow(icon: FontAwesomeIcons.barcode, label: "Boleto", isSelected: _selectedMethod == 3, onTap: () => setState(() => _selectedMethod = 3))])), ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(30)), child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10), child: Container(padding: const EdgeInsets.all(30), decoration: BoxDecoration(color: Colors.white.withOpacity(0.1), borderRadius: const BorderRadius.vertical(top: Radius.circular(30)), border: Border(top: BorderSide(color: Colors.white.withOpacity(0.2)))), child: SizedBox(width: double.infinity, height: 60, child: ElevatedButton(
            onPressed: _selectedMethod == 0 ? null : () { 
              Navigator.push(context, MaterialPageRoute(builder: (context) => const OrderSuccessScreen())); 
            }, 
            style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF800020), disabledBackgroundColor: Colors.grey.withOpacity(0.3), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: Text("Confirmar método de pagamento", style: GoogleFonts.poppins(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)))))))])
        ],
      ),
    );
  }
...
class _PaymentOptionRow extends StatelessWidget {
  ...
  const _PaymentOptionRow({required this.icon, required this.label, required this.isSelected, required this.onTap});
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: onTap,
      child: ClipRRect(borderRadius: BorderRadius.circular(20), child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10), child: Container(padding: const EdgeInsets.symmetric(vertical: 20, horizontal: 20), decoration: BoxDecoration(color: isSelected ? Colors.white.withOpacity(0.15) : Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(20), border: Border.all(color: isSelected ? const Color(0xFF800020) : Colors.white.withOpacity(0.1), width: isSelected ? 2 : 1)), child: Row(children: [FaIcon(icon, color: Colors.white70, size: 24), const SizedBox(width: 20), Expanded(child: Text(label, style: GoogleFonts.poppins(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w500))), Container(width: 28, height: 28, decoration: BoxDecoration(color: isSelected ? const Color(0xFF800020) : Colors.transparent, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.white70, width: 2)), child: isSelected ? const Icon(Icons.check, size: 20, color: Colors.white) : null)])))),
    );
  }
}

// =========================================================
// TELA 8: ORDER SUCCESS (CONFIRMAÇÃO DO PEDIDO)
// =========================================================
              ...
              child: ClipRRect(
                ...
                child: BackdropFilter(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: const Color(0xFFE8DCCA).withOpacity(0.9), 
                      borderRadius: BorderRadius.circular(20),
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.notifications_active_outlined, size: 40, color: Colors.black54),
                        ),
                        const SizedBox(height: 20),
                          ...
                          child: Column(
                            children: [
                              RichText(
                                textAlign: TextAlign.center,
                                text: TextSpan(
                                  style: GoogleFonts.poppins(color: const Color(0xFF3E2723), fontSize: 14),
                                  children: const [
                                    TextSpan(text: "Nosso Sistema já recebeu seu pedido e o mesmo está "),
                                    TextSpan(text: "em produção para sair para a entrega.", style: TextStyle(fontWeight: FontWeight.bold)),
                                  ]
                                ),
                              ),
                              const SizedBox(height: 15),
                              Text("Obrigado pela preferência.", style: GoogleFonts.greatVibes(fontSize: 24, color: const Color(0xFF3E2723))),
                            ],
                          ),
                        ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            child: Text("FINALIZAR", style: GoogleFonts.poppins(color: Colors.white, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                          ),
                        ),
